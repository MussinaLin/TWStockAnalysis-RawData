...
原設計把「新上市櫃前五日會算出不適用的值」列為不處理的已知限制。端到端驗收時
發現兩件當初沒掌握的事實，故改為部分處理：

1. **櫃買其實有明示**：新掛牌股在無漲跌幅限制期間，TPEX `dailyQuotes` 的
   「次日漲停價 / 次日跌停價」會給哨兵值 `9995.00` / `0.01`
   （國外成分 ETF 則是 `9999.95` / `0.01`）。原設計假設得自己偵測，其實答案是現成的。
2. **影響是靜默的**：這些列會寫入看起來合理的假區間。2025-01 ~ 2025-05 實測有
   6 檔新掛牌、15 筆首五日資料被寫入假值。當時未產生假漲停訊號，但那是巧合 ——
   收盤價撞到假邊界就會讓下游拿到不存在的漲停訊號。

**採用的處理方式（自我否證）**：`_price_limits` 算完後，用當日 `high` / `low` /
`close` 檢查區間是否有拘束力 —— 有漲跌幅限制時不可能成交在區間外，所以只要任一
價格落在區間外就改寫 NULL。不需新增資料來源，兩條寫入路徑共用。

**殘留限制**：只擋得掉成交價真的衝出假區間的情形。上述 15 筆中擋下 4 筆，
其餘 11 筆當日波動落在假區間內，無內部證據可否證。已寫入 README，
下游若在意可自行排除個股上市櫃後的前五個交易日。

未採用「讀櫃買哨兵值」的方案：那是「次日」的欄位，需跨日帶狀態，
且上市股沒有等價來源（TWSE 不公告次日漲跌停），涵蓋不完整。

## 明確不做

- 不處理除權息日的參考價補正（需另接 TWSE 除權除息計算結果表），該日寫 NULL。
- 不存 boolean 旗標，不存漲跌價差原值，不存參考價 —— 只存兩個價格欄位。
- 不涵蓋興櫃與 ETF：標的池來自上市櫃公司基本資料，本就不含這兩類。
